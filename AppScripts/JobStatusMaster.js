...
$(document).ready(function () {
    setSession("Page", "JobstatusMaster");
    ...
    $('#lblPagetitle').text("Config Tables-Job Status");
...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAddjobstatus").val();
        var status = "";
        var mastertype = "Jobstatus";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDJOBSTATUSID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATEJOBSTATUSID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETEJOBSTATUSID", strdata);

}


function getJobstatusidlist() {

    var strdata = { "mastertype": "Jobstatus" };

    common_api_ajax_request("api/Masterview", "JOBSTATUSLIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDJOBSTATUSID" || key == "DELETEJOBSTATUSID" || key == "UPDATEJOBSTATUSID") {
        $("#txtAddjobstatus").val("");
        if (key == "ADDJOBSTATUSID") {
            $("#div-message").text('Jobstatus saved successfully');
        }
        else if (key == "DELETEJOBSTATUSNID") {
            $("#div-message").text('Jobstatus deleted successfully');
        }
        else if (key == "UPDATEJOBSTATUSID") {
            $("#div-message").text('Jobstatus status updated successfully');
        }
    ...
    $.each(data, function (key, item) {
        ...
        if (item.masterid == "") {
            ...
            strTable += "<tr class='divControl'>" +
                ...
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblJobstatusnrowid' style='display:none;'>" + item.masterid + "</label>" +
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblJobstatusvalue'>" + item.mastername + "</label>" +
                "</div>" +
                "</div >" +
                "</td >" +
                statusTD +
...
function Validatetxt() {
    var count = 0;
    var emptycount = 0;
    if ($('#txtAddjobstatus').val() == "") {
        $('#txtAddjobstatus').addClass("is-invalid");
        $('#txtAddjobstatus').focus();
        emptycount++;
    }
    else {
        $('#txtAddjobstatus').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAddjobstatus').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
