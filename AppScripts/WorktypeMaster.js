...
$(document).ready(function () {
    setSession("Page", "WorktypeMaster");
    ...
    $('#lblPagetitle').text("Config Tables-Work Type");
...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAddworktype").val();
        var status = "";
        var mastertype = "Worktype";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDWORKTYPEID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATEWORKTYPEID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETEWORKTYPEID", strdata);

}


function getWorktypeidlist() {

    var strdata = { "mastertype": "Worktype" };

    common_api_ajax_request("api/Masterview", "WORKTYPELIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDWORKTYPEID" || key == "DELETEWORKTYPEID" || key == "UPDATEWORKTYPEID") {
        $("#txtAddworktype").val("");
        if (key == "ADDWORKTYPEID") {
            $("#div-message").text('Worktype saved successfully');
        }
        else if (key == "DELETEWORKTYPEID") {
            $("#div-message").text('Worktype deleted successfully');
        }
        else if (key == "UPDATEWORKTYPEID") {
            $("#div-message").text('Worktype status updated successfully');
        }
    ...
    $.each(data, function (key, item) {
        ...
        if (item.masterid == "") {
            ...
            strTable += "<tr class='divControl'>" +
                ...
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblWorktyperowid' style='display:none;'>" + item.masterid + "</label>" +
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblWorktypevalue'>" + item.mastername + "</label>" +
                "</div>" +
                "</div >" +
                "</td >" +
                statusTD +
...
function Validatetxt() {
    var count = 0;
    var emptycount = 0;
    if ($('#txtAddworktype').val() == "") {
        $('#txtAddworktype').addClass("is-invalid");
        $('#txtAddworktype').focus();
        emptycount++;
    }
    else {
        $('#txtAddworktype').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAddworktype').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
