...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAddspecilization").val();
        var status = "";
        var mastertype = "Specialization";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDSPECIALIZATIONID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATESPECIALIZATIONID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETESPECIALIZATIONID", strdata);

}


function getSpecializationidlist() {

    var strdata = { "mastertype": "Specialization" };

    common_api_ajax_request("api/Masterview", "SPECIALIZATIONLIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDSPECIALIZATIONID" || key == "DELETESPECIALIZATIONID" || key == "UPDATESPECIALIZATIONID") {
        $("#txtAddspecilization").val("");
        if (key == "ADDSPECIALIZATIONID") {
            $("#div-message").text('Specialization saved successfully');
        }
        else if (key == "DELETESPECIALIZATIONID") {
            $("#div-message").text('Specialization deleted successfully');
        }
        else if (key == "UPDATESPECIALIZATIONID") {
            $("#div-message").text('Specialization status updated successfully');
        }
...
function Validatetxt() {
    var count = 0;
    var emptycount = 0;
    if ($('#txtAddspecilization').val() == "") {
        $('#txtAddspecilization').addClass("is-invalid");
        $('#txtAddspecilization').focus();
        emptycount++;
    }
    else {
        $('#txtAddspecilization').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAddspecilization').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
