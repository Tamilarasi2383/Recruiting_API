...
$(document).ready(function () {
    setSession("Page", "DesignationMaster");
...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAdddesignation").val();
        var status = "";
        var mastertype = "Designation";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDDESIGNATIONID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATEDESIGNATIONID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETEDESIGNATIONID", strdata);

}


function getDesignationidlist() {

    var strdata = { "mastertype": "Designation" };

    common_api_ajax_request("api/Masterview", "DESIGNATIONLIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDDESIGNATIONID" || key == "DELETEDESIGNATIONID" || key == "UPDATEDESIGNATIONID") {
        $("#txtAdddesignation").val("");
        if (key == "ADDDESIGNATIONID") {
            $("#div-message").text('Designation saved successfully');
        }
        else if (key == "DELETEDESIGNATIONID") {
            $("#div-message").text('Designation deleted successfully');
        }
        else if (key == "UPDATEDESIGNATIONID") {
            $("#div-message").text('Designation status updated successfully');
        }
    ...
    $.each(data, function (key, item) {
        ...
        if (item.masterid == "") {
            ...
            strTable += "<tr class='divControl'>" +
                ...
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblDesignationrowid' style='display:none;'>" + item.masterid + "</label>" +
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblDesignationvalue'>" + item.mastername + "</label>" +
                "</div>" +
                "</div >" +
                "</td >" +
                statusTD +
...
function Validatetxt() {
    var count = 0;
    var emptycount = 0;
    if ($('#txtAdddesignation').val() == "") {
        $('#txtAdddesignation').addClass("is-invalid");
        $('#txtAdddesignation').focus();
        emptycount++;
    }
    else {
        $('#txtAdddesignation').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAdddesignation').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
