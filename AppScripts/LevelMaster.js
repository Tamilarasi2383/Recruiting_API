...
$(document).ready(function () {
    ...
    $('#lblPagetitle').text("Config Tables-Level");
...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAddlevel").val();
        var status = "";
        var mastertype = "Level";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDLEVELID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATELEVELID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETELEVELID", strdata);
...
function getLevelidlist() {
    ...
    var strdata = { "mastertype": "Level" };

    common_api_ajax_request("api/Masterview", "LEVELLIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDLEVELID" || key == "DELETELEVELID" || key == "UPDATELEVELID") {
        $("#txtAddlevel").val("");
        if (key == "ADDLEVELID") {
            $("#div-message").text('Level saved successfully');
        }
        else if (key == "DELETELEVELID") {
            $("#div-message").text('Level deleted successfully');
        }
        else if (key == "UPDATELEVELID") {
            $("#div-message").text('Level status updated successfully');
        }
        $('#hdnrowid').val("");
        $("#hdnstatus").val("");
    ...
    $.each(data, function (key, item) {
        ...
        if (item.masterid == "") {
            ...
            strTable += "<tr class='divControl'>" +
                ...
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblLevelrowid' style='display:none;'>" + item.masterid + "</label>" +
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblLevelvalue'>" + item.mastername + "</label>" +
                "</div>" +
                "</div >" +
                "</td >" +
                statusTD +
...
function Validatetxt() {
    ...
    var emptycount = 0;
    if ($('#txtAddlevel').val() == "") {
        $('#txtAddlevel').addClass("is-invalid");
        $('#txtAddlevel').focus();
        emptycount++;
    }
    else {
        $('#txtAddlevel').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAddlevel').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
