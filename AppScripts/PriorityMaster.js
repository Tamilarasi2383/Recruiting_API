...
$(document).ready(function () {
    setSession("Page", "PriorityMaster");
    ...
    $('#lblPagetitle').text("Config Tables-Priority");
...
function savetxtid() {
    ...
    if (Validatetxt() == true) {
        var fieldname = $("#txtAddpriorty").val();
        var status = "";
        var mastertype = "Priority";
        var mode = "";
        var rowid = "";
        if ($('#hdnrowid').val() == "") {
            mode = "I";
        ...
        common_api_ajax_request("api/Masterinsert", "ADDPRIORITYID", strdata);
    }
...
function confirmUpdate(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "UPDATEPRIORITYID", strdata);
...
function confirmDelete(rowid) {
    ...
    common_api_ajax_request("api/Masterinsert", "DELETEPRIORITYID", strdata);

}


function getPriorityidlist() {

    var strdata = { "mastertype": "Priority" };

    common_api_ajax_request("api/Masterview", "PRIORITYLIST", strdata);
...
function successCallBack(key, value) {
    ...
    else if (key == "ADDPRIORITYID" || key == "DELETEPRIORITYID" || key == "UPDATEPRIORITYID") {
        $("#txtAddpriorty").val("");
        if (key == "ADDPRIORITYID") {
            $("#div-message").text('Priority saved successfully');
        }
        else if (key == "DELETEPRIORITYID") {
            $("#div-message").text('Priority deleted successfully');
        }
        else if (key == "UPDATEPRIORITYID") {
            $("#div-message").text('Priority status updated successfully');
        }
    ...
    $.each(data, function (key, item) {
        ...
        if (item.masterid == "") {
            ...
            strTable += "<tr class='divControl'>" +
                ...
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblPriorityrowid' style='display:none;'>" + item.masterid + "</label>" +
                "<label class=text-secondary 'mb-0 text-sm h6' id='lblPriorityvalue'>" + item.mastername + "</label>" +
                "</div>" +
                "</div >" +
                "</td >" +
                statusTD +
...
function Validatetxt() {
    var count = 0;
    var emptycount = 0;
    if ($('#txtAddpriorty').val() == "") {
        $('#txtAddpriorty').addClass("is-invalid");
        $('#txtAddpriorty').focus();
        emptycount++;
    }
    else {
        $('#txtAddpriorty').removeClass("is-invalid");
        count++;
    }
...
function editRow(Expbutton) {
    ...
    $('#txtAddpriorty').val(table.rows[rowid].cells[0].getElementsByTagName("label")[1].innerHTML);
