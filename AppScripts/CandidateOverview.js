$(document).ready(function () {
    ...
    setSession("Page", "CandidateOverview");
...
function getcandidateoverviewList(tileValue) {
    ...
    if (TWE_Id != "") {
        common_api_ajax_request("api/CandidateOveview", "CANDOVERVIEW", strdata);
    }
}
...
function Createactivejobs(lstjob) {
    ...
    for (var i = 0; i < lstjob.length; i++) {
        ...
        str += "<li class='list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg' >" +
            ...
            "<span class='text-xs'><span id='lblMonthvalue'>" + lstjob[i].JobDuration + "</span> | <span id='lblType'>" + lstjob[i].JobType + "</span> | <span id='lblCity'>" + lstjob[i].JobCity + "</span> , <span id='lblState'>" + lstjob[i].JobState + "</span> , <span id='lblCountry'>" + lstjob[i].JobCountry + "</span></span>" +
            "</div>" +
            "<div class='d-flex align-items-center text-sm'>" +
            "<button class='btn btn-link text-dark text-sm mb-0 px-0 ms-4' onclick=window.location.href='FollowupPage.aspx?id=" + lstjob[i].Jobcode+"'><i class='text-lg me-1' id='btnFollow' ;></i>Follow</button>" +
            "</div>" +
            "</li >";
    }
    ...
    $.each(data, function (key, item) {
        if (item.Jobcode == "") {
            ...
            strTable += "<tr style='cursor:pointer;' class='divControl'><td onclick=gotocandidatepage('" + item.candidateid +"');> <div class='d-flex px-2 py-1'>" +
                ...
                "<p class='text-xs text-secondary mb-0'><a href='/cdn-cgi/l/email-protection' class='__cf_email__' data-cfemail='96fcf9fef8d6f5e4f3f7e2ffe0f3bbe2fffbb8f5f9fb'>" + item.candidateemailid + " | " + item.candidatemobno + " | " + "$" + item.ratePerHr + "/Hr" + " - " + item.type + "</a></p>" +
                "</div ></div ></td > ";
